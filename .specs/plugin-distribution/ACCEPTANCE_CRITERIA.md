...
**WHEN** release eligibility is evaluated **THEN** it SHALL require current passed mandatory evidence for every FR-1 through FR-12, bound to one commit, OMP pin, platform fixture, catalog digest, artifact digest, and candidate version; for `0.1.0` that set SHALL include version consistency, clean install, reload, fresh-session activation and invocation, uninstall with fresh-session absence, and reinstall with fresh-session invocation while marking prior-version upgrade/rollback inapplicable; beginning with the first subsequent release it SHALL additionally require passed upgrade-from-prior and rollback-to-prior evidence. **IF** any applicable requirement evidence is missing, failed, blocked, stale, mismatched, or represented only by a passing stage/job summary **THEN** eligibility SHALL be `blocked`.
