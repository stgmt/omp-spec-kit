...
- Added FR-15 contained step-binding index (`STEP_BINDING`, `BINDS_STEP`, `STEP_UNDEFINED`/`STEP_AMBIGUOUS`) over allowlisted `tests/step-definitions/**/*.js|mjs`. It is **not** a `kernel-v0.2`/`kernel-v0.3` required check; `CHK-FR15-01` (`kernel-step-bindings`) is the unblocker for `spec-lsp` step diagnostics. Callers query via findNodes/getEdges/diagnostics; this FR adds no MCP tool (unfrozen: not an eternal eight-tool freeze).
...
- Added FR-17 MCP adapter document and preflight I/O (`list_spec_docs`, `read_spec_doc`, `read_attachment`, `mcp_preflight`; census rows 17–20). Read-only, FR-7 containment, not query-service ops, projected by MCP later, no second graph. Not a v0.2/v0.3 required check.
- Traceability is 17 FR / 17 AC / 17 scenarios; v0.2/v0.3 gates still exclude FR-15, FR-16, and FR-17.
