...
- Added FR-15 contained step-binding index (`STEP_BINDING`, `BINDS_STEP`, `STEP_UNDEFINED`/`STEP_AMBIGUOUS`) over allowlisted `tests/step-definitions/**/*.js|mjs`. It is **not** a `kernel-v0.2`/`kernel-v0.3` required check; `CHK-FR15-01` (`kernel-step-bindings`) is the unblocker for `spec-lsp` step diagnostics. MCP stays eight tools.
- Traceability is 15 FR / 15 AC / 15 scenarios; v0.2/v0.3 gates still exclude FR-15.
