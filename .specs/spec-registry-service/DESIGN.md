...
- **Enforcement pair**:
  - specs repo: pushes restricted to the service bot (GitHub ruleset / branch protection). Break-glass admin pushes remain possible and are caught by drift reporting.
...
- **Transaction artifacts must not be committed.** The existing lock (`.specs/.omp-spec-kit-write.lock`) and staging dir (`.specs/.omp-spec-kit-staging/`) live inside each tracked `.specs/` tree — in the specs repo they would land in git. The repo's root `.gitignore` (the only non-spec file) must exclude `**/.omp-spec-kit-*`; alternatively the paths relocate outside `.specs` (kernel change — decide in implementation).
