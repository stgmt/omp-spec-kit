...
**Independent Test:** `SCEN-MRI-001` installs the built package in project-a with no root override and proves that `spec_overview` returns project-a data but excludes package-decoy and project-b data.

**Acceptance Scenarios:**

Given project-a, project-b, and the package folder each contain distinct `.specs` fixtures
When a fresh OMP process discovers the installed v0.3.1 MCP server from project-a without `OMP_SPEC_KIT_ROOT`
...
**Independent Test:** `SCEN-MRI-002` sends raw newline-delimited JSON-RPC frames to the built server and checks response count, id, code, and complete stdout framing.
...
Then the server emits exactly one JSON-RPC `-32700` response with null id and no non-protocol stdout
...
**Independent Test:** `SCEN-MRI-003` changes one candidate identity at a time—tag commit, archive byte, receipt, upgrade, rollback, or asset—and asserts release eligibility fails before publish.

**Acceptance Scenarios:**

Given a v0.3.1 candidate has all current required receipts and a matching peeled tag commit
...
As an existing user, I want the v0.3.0 issue and v0.3.1 fix described plainly, so that I know whether to upgrade and do not rely on stale claims.
...
**Independent Test:** `SCEN-MRI-004` validates generated v0.3.1 notes and documentation against the verified candidate version/evidence and checks the v0.3.0 advisory target.

**Acceptance Scenarios:**

Given v0.3.0 remains historically published
When a user opens its release page or advisory
Then it is marked superseded for the MCP root defect and directs the user to v0.3.1

Given v0.3.1 has passed all release gates
...
As a release maintainer, I want v0.3.1 eligibility to require observed upgrade and rollback receipts, so that a release cannot be called recoverable from a job summary alone.
...
**Independent Test:** `SCEN-MRI-004` rejects a missing or mismatched lifecycle receipt before eligibility.
...
**Independent Test:** `SCEN-MRI-005` tampers with the archive and observes release eligibility fail before release mutation.
