...
`SCEN-MRI-018` writes a structurally complete producer matrix with input trust `github-artifact-attestation` and evaluates it inside the BDD container. The container has no `gh` binary, so the evaluator's independent verifier (`gh attestation verify ... --signer-workflow <repo>/.github/workflows/distribution-evidence.yml --source-ref refs/tags/<tag>`) deterministically fails and the scenario asserts public release stays blocked with a `distribution-producer-attestation-unverified:` reason while MRI remains independently eligible. This proves fail-closed semantics for the trusted path without mocking `gh`; no real Sigstore verification is claimed in tests. The production trust root is exercised only by `.github/workflows/distribution-evidence.yml` (build bundle from real CI outputs -> `actions/attest@v4`) and `.github/workflows/release.yml` (cross-run artifact download -> `--distribution-trust github-artifact-attestation`). Claims whose lifecycle producers do not exist in CI (install, reload, fresh-session-activation, upgrade, rollback, uninstall-preservation, reinstall, release-transaction, evidence-honesty) are omitted by the builder rather than fabricated; their missing matrix cells keep any current candidate blocked.
