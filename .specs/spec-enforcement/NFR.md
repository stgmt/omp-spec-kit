...
Equal normalized inputs and equal filesystem state SHALL produce byte-identical decisions. Public codes are limited to `AUTHORING_TOOL_ALLOWED`, `NON_SPEC_ALLOWED`, `RAW_SPEC_WRITE`, and `TARGET_INDETERMINATE`. An exception or unresolved filesystem condition SHALL block.
