...
The capability will live in the existing `plugins/omp-spec-kit` package and existing MCP server. MCP is the sole agent-facing authoring adapter; the existing OMP extension remains read-only. The capability must not create another marketplace entry, plugin package, extension control plane, or direct writer. Read-only kernel access precedes mutation only for ordinary unlinked roots. Every write begins with a read-only validated proposal, requires a separate authenticated review of the complete proposal ID/hash, uses current expected-hash compare-and-swap (CAS), and commits a same-spec multi-document transaction atomically. `apply_transaction` cannot accept raw edits or preview and commit in one call.

The agent-facing authoring API is MCP ([FR-14](FR.md#fr-14-generator-port-mutation-names)). Seventeen v1 facade names compile to the exact proposal-first mapping table in the schema; normal/rebaseline commits use a review-only call followed by a later matching commit-only call; closed `propose_patch` modes and `apply_spec_transaction` phases also route status, cancel, retained recovery and rebaseline operations without extra tool names. Seven names (`set_spec_status`, create/archive/doc CRUD/backlog helpers) remain unregistered v2-later entries in the capability manifest, not invokable v1 refusals and not DROP. `authoring-mcp@1` requires accepted FR-13 plus CHK-FR14-01; no authoring name appears in historical v0.3.
...
- OMP extension lifecycle at the implementation pin: https://github.com/can1357/oh-my-pi/blob/8500092296621a6826b7136e840f8a59ea338958/docs/extensions.md
- OMP marketplace lifecycle at the implementation pin: https://github.com/can1357/oh-my-pi/blob/8500092296621a6826b7136e840f8a59ea338958/docs/marketplace.md
