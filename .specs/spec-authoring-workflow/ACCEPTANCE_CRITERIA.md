...
**Requirement:** [FR-1](FR.md#fr-1-deferred-eligibility-and-single-plugin-boundary)
...
**Requirement:** [FR-2](FR.md#fr-2-proposal-and-bounded-diff-preview)
...
**WHEN** an apply facade is called with `phase: "review"` for a complete validated proposal **THEN** it SHALL perform only the authenticated `VALIDATED→REVIEWED` transition and return without writing; **WHEN** a later call to an apply facade uses `phase: "commit"` **THEN** `apply_transaction` SHALL consume that separately reviewed, unexpired `proposalId`/`proposalHash` and current expected hashes for every and only proposal document plus the base snapshot; **IF** one call requests both phases, raw edits, an unreviewed or truncated proposal, an omitted/extra/mismatched hash, or an expired proposal **THEN** it SHALL refuse before staging.

## AC-3.1

**Requirement:** [FR-3](FR.md#fr-3-expected-hash-cas-and-concurrency)
...
**Requirement:** [FR-4](FR.md#fr-4-validation-before-write)
...
**Requirement:** [FR-5](FR.md#fr-5-root-containment-and-deny-by-default-symlink-policy)
...
**Requirement:** [FR-6](FR.md#fr-6-atomic-same-spec-transaction-rollback-and-recovery)
...
**Requirement:** [FR-7](FR.md#fr-7-anchor-safe-editing)
...
**Requirement:** [FR-8](FR.md#fr-8-guarded-taskstatus-lifecycle)
...
**Requirement:** [FR-9](FR.md#fr-9-provenance-and-redacted-audit-evidence)
...
**Requirement:** [FR-10](FR.md#fr-10-typed-request-result-and-error-contract)
...
**WHEN** an authoring request reaches the MCP server **THEN** its facade SHALL delegate to the one shared service and produce the closed service result; **AND WHEN** the existing OMP extension inventory is inspected **THEN** it SHALL remain read-only and expose no authoring facade, writer, or adapter-specific mutation semantic.

## AC-11.1

**Requirement:** [FR-11](FR.md#fr-11-mutation-resistance-release-gate)
...
**Requirement:** [FR-12](FR.md#fr-12-no-bypass-hidden-state-or-dev-pomogator-integration)

**WHEN** runtime/package inventory is inspected **THEN** it SHALL find one existing plugin package, one read-only extension entry, one MCP authoring adapter over one shared transaction authority, and zero direct/fallback writers.
...
**Requirement:** [FR-13](FR.md#fr-13-aggregate-authoring-eligibility)
...
**Requirement:** [FR-14](FR.md#fr-14-generator-port-mutation-names)

**WHEN** the generator-port mutation census is projected **THEN** the seventeen v1 MCP facades SHALL compile exactly to the proposal-first mappings in the schema; every normal/rebaseline commit SHALL require a separately REVIEWED matching proposal; `propose_patch` status/patch/rebaseline modes and `apply_spec_transaction` review/commit/cancel/recover-retained/commit-rebaseline phases SHALL make every closed service request reachable without combining transitions; the seven v2 names (`set_spec_status`, `create_spec`, `archive_spec`, `delete_spec_doc`, `rename_spec_doc`, `add_backlog_task`, `register_incident_backlog`) SHALL remain unregistered later names listed in `unsupportedLaterNames`, not invokable v1 refusals and not DROP; none SHALL appear on historical v0.3; and `authoring-mcp@1` eligibility SHALL require accepted FR-13 plus CHK-FR14-01.
