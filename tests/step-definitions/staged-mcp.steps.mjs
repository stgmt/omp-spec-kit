...
When("the registry and every handler are called", { timeout: 30000 }, async function () {
...
Then(/the registry has exactly (?:10|11) names and every call has a bounded envelope/, function () {
  assert.equal(this.stagedMcp.listedNames.length, 10);
  assert.equal(new Set(this.stagedMcp.listedNames).size, 10);
...
When("an authoring spec patch is executed with dryRun false", { timeout: 30000 }, async function () {
  ...
  const applied = await this.stagedMcp.server.request("tools/call", {
    ...
    arguments: {
      ...
      spec: "product",
      reason: "verify spec patch applies",
  ...
  this.stagedMcp.rename = { applied: renameApplied.result.structuredContent };
});

Then("the applied spec patch changes the temporary document, section edits preserve it, and direct spec writes are refused", async function () {
  assert.equal(this.stagedMcp.apply.ok, true);
  assert.equal(this.stagedMcp.append.applied.ok, true);
  assert.equal(this.stagedMcp.rename.applied.ok, true);
  const content = await readFile(path.join(this.stagedMcp.root, ".specs", "product", "README.md"), "utf8");
  assert.ok(content.includes("BDD authoring marker"));
  assert.ok(content.includes("BDD append marker"));
  assert.ok(content.includes("Current product status renamed"));
  const blocked = classifyToolCall({ toolName: "write", cwd: this.stagedMcp.root, input: { path: ".specs/product/README.md" } }, { root: this.stagedMcp.root });
  assert.equal(blocked.action, "block");
  assert.equal(blocked.code, "RAW_SPEC_WRITE");
  assert.equal(blocked.mismatchField, null);
  const allowed = classifyToolCall({ toolName: "mcp__omp_spec_kit_spec_patch", input: {} });
  const removedMinted = classifyToolCall({ toolName: "mcp__omp_spec_kit_apply_spec_change", input: {} });
  const removedShort = classifyToolCall({ toolName: "apply_spec_change", input: {} });
  assert.equal(allowed.action, "allow");
  assert.notEqual(removedMinted.action, "allow");
  assert.notEqual(removedShort.action, "allow");
});

When("authoring safety guards are exercised", { timeout: 30000 }, async function () {
...
When("the read server receives alias and unknown-field calls", { timeout: 30000 }, async function () {
  await this.stagedMcp.server.close();
  this.stagedMcp.server = spawnMcpServer({
    serverPath: SERVER_PATH,
    root: this.stagedMcp.root,
...
Then("aliases work, unknown fields fail, and removed tools stay unknown", function () {
  ...
  assert.equal(this.stagedMcp.inputResults.hiddenNames.includes("spec_patch"), true, "spec_patch must be registered");
  assert.equal(this.stagedMcp.inputResults.hiddenNames.includes("spec_propose_patch"), false, "spec_propose_patch must not be registered");
  assert.equal(this.stagedMcp.inputResults.hiddenNames.includes("apply_proposed_patch"), false, "apply_proposed_patch must not be registered");
  for (const removed of ["spec_propose_patch", "apply_proposed_patch", "propose_patch", "create_spec", "list_specs", "spec_inventory", "apply_spec_change", "apply_spec_transaction", "apply_spec_repairs", "append_to_section", "insert_after_heading", "insert_at_eof", "replace_in_section", "propose_spec_change", "propose_spec_repairs", "list_phase_tasks", "propose_requirement_contract"]) {
    assert.equal(this.stagedMcp.inputResults.hiddenNames.includes(removed), false, removed + " must be gone");
  }
});

When("an incomplete evidence stream is queried", { timeout: 30000 }, async function () {
  await this.stagedMcp.server.close();
...
When("a new specification is created and archived through the spec patch door", { timeout: 30000 }, async function () {
  ...
  this.stagedMcp.archive = { created: createdApplied, archivedApply };
});
...
Then("the archive move is committed and the original directory is absent", async function () {
  ...
  assert.equal(this.stagedMcp.archive.archivedApply.data.outcome, "APPLIED");
  assert.deepStrictEqual(this.stagedMcp.archive.archivedApply.data.receipt.archive.spec, "archive-bdd");
  assert.deepStrictEqual(this.stagedMcp.archive.archivedApply.data.receipt.archive.destination, ".specs/archive/archive-bdd");
...
When("the staged OMP extension registry is inspected", { timeout: 30000 }, async function () {
...
Then("spec patch is marked as mutating and 9 tools remain read-only", function () {
  ...
  assert.equal(tools.size, 10, "MCP server must expose the single 10-tool surface");
  assert.equal(tools.get("spec_patch")?.annotations?.readOnlyHint, false);
...
async function runToolE2EPhase(world, phase) {
  ...
  return runEvidenceE2E({
    ...
    callTool: (name, args) => world.server.request("tools/call", { name, arguments: args }),
...
Then("the inventory contains the exact 10-tool surface", async function () {
  await runToolE2E({
    listTools: () => this.stagedMcp.server.request("tools/list"),
    callTool: (name, args) => this.stagedMcp.server.request("tools/call", { name, arguments: args }),
    projectRoot: this.stagedMcp.root,
    repositoryRoot: REPOSITORY_ROOT,
    phase: "inventory",
  });
...
When("the mutation and freshness matrix is exercised", { timeout: 60000 }, async function () {
...
When("the read-only matrix is exercised", { timeout: 30000 }, async function () {
  this.toolE2E = await runToolE2EPhase(this.stagedMcp, "boundary");
});
...
When("MCP envelope recovery cases are exercised", { timeout: 30000 }, async function () {
  ...
  const conflict = await request("spec_patch", {
    requestId: "bdd-recovery-conflict-apply",
    intent: "patch",
    dryRun: false,
    repositoryRootFingerprint: fingerprint,
    spec: "product",
    reason: "probe conflict",
    operations: [{ kind: "insert_at_eof", document: "README.md", text: "conflict probe\n", expectedSha: "0".repeat(64) }],
  });
  const targetBlocked = classifyToolCall({ toolName: "write", cwd: this.stagedMcp.root, input: { path: "" } }, { root: this.stagedMcp.root });
...
Then("stale cursor, conflict, and target indeterminate recoveries are bounded and actionable", function () {
  ...
  assert.ok(stale.result.structuredContent.error.message.includes("Recovery: retry the same list operation without cursor to obtain a fresh page, then continue with the returned nextCursor."));
  ...
  assert.ok(conflict.result.structuredContent.data.error.message.includes("Recovery: refresh spec_catalog with overview view, resolve the reported conflict, review a fresh preview with dryRun: true, then call spec_patch with dryRun: false and a new requestId."));
  ...
  assert.ok(targetBlocked.reason.includes("Recovery: provide one explicit repository-relative target, or use spec_patch with dryRun: true for preview or dryRun: false to apply."));
...
When("the additive registry, evidence states, and safe authoring are exercised", { timeout: 60000 }, async function () {
  await this.stagedMcp.server.close();
  this.stagedMcp.server = spawnMcpServer({
    serverPath: SERVER_PATH,
    root: this.stagedMcp.root,
  ...
  const patchArgs = {
    ...
    spec: "product",
    reason: "authoring proof",
    operations: [{ kind: "insert_at_eof", document: "README.md", text: "authoring marker" }],
  };
  const applied = await this.stagedMcp.server.request("tools/call", { name: "spec_patch", arguments: patchArgs });
  const replay = await this.stagedMcp.server.request("tools/call", { name: "spec_patch", arguments: { ...patchArgs, requestId: "v05-replay" } });
  const proposal = applied.result.structuredContent;
...
Then("the surface exposes 10 bounded tools, preserves authoring, and refuses stale evidence", async function () {
...
const ALL_SUPERSEDED_38_TOOLS = Object.freeze([
  "spec_inventory",
  "spec_get_node",
  "spec_find_nodes",
  "spec_get_edges",
...
When("all 38 superseded tool names are invoked individually", { timeout: 60000 }, async function () {
  this.supersededResults = [];
  for (const name of ALL_SUPERSEDED_38_TOOLS) {
...
Then("every superseded tool returns JSON-RPC error -32602 without fallback shims", function () {
  assert.equal(this.supersededResults.length, 38);
...
When("all consolidated branches and all 13 spec patch intents are exercised", { timeout: 60000 }, async function () {
  ...
  const calls = [
    ...
    // 10. spec_patch - all 13 intents
    ["spec_patch", { intent: "patch", repositoryRootFingerprint: fingerprint, spec: "product", reason: "bdd test", requestId: "bdd-int-patch", operations: [{ kind: "insert_at_eof", document: "README.md", text: "bdd" }] }, "specPatch", null],
    ["spec_patch", { intent: "amendRequirement", spec: "product", requirement: "FR-1", body: "amend", reason: "amend reason", requestId: "bdd-int-amend" }, "specPatch", null],
    ["spec_patch", { intent: "addAcceptanceCriterion", spec: "product", requirement: "FR-1", criterion: "crit", reason: "ac reason", requestId: "bdd-int-ac" }, "specPatch", null],
    ["spec_patch", { intent: "addPhase", spec: "product", title: "New Phase", reason: "phase reason", requestId: "bdd-int-phase" }, "specPatch", null],
    ["spec_patch", { intent: "setEntityStatus", spec: "product", entity: "TASK-1", status: "in-progress", reason: "status reason", requestId: "bdd-int-status" }, "specPatch", null],
    ["spec_patch", { intent: "setSpecStatus", spec: "product", status: "active", reason: "spec status reason", requestId: "bdd-int-spec-status" }, "specPatch", null],
    ["spec_patch", { intent: "setRequirementMetadata", spec: "product", requirement: "FR-1", metadata: { schemaVersion: 1, verificationMethod: "test" }, reason: "meta reason", requestId: "bdd-int-meta" }, "specPatch", null],
    ["spec_patch", { intent: "deleteSpecDoc", spec: "temp-e2e-spec", doc: "TASKS.md", reason: "delete doc reason", requestId: "bdd-int-del" }, "specPatch", null],
    ["spec_patch", { intent: "renameSpecDoc", spec: "temp-e2e-spec", doc: "TASKS.md", newDoc: "FIXTURES.md", reason: "rename doc reason", requestId: "bdd-int-ren" }, "specPatch", null],
    ["spec_patch", { intent: "createSpec", spec: "temp-spec-123", title: "Temp Spec", reason: "create reason", requestId: "bdd-int-create" }, "specPatch", null],
