...
function expectedManagerToolCount(version) {
  if (version === "2.7.0" || version === "2.6.2" || version === "2.6.1" || version === "2.6.0" || version === "2.5.0" || version === "2.4.0" || version === "2.3.0" || version === "2.2.1" || version === "2.2.0" || version === "2.1.0" || version === "2.0.0" || version === "1.4.0" || version === "1.3.2" || version === "1.3.1" || version === "1.3.0" || version === "1.2.0" || version === "1.1.0" || version === "1.0.2" || version === "1.0.1" || version === "1.0.0" || version === "0.10.2") return 10;
...
export async function evaluateRelease({ candidatePath, evidencePath, tag, repositoryRoot = defaultRepositoryRoot, resolveTagCommit = peelTagCommit }) {
  ...
  const appZipPath = path.resolve(candidateDirectory, candidate.youtrackApp.file);
  if (appZipPath !== candidateDirectory && !appZipPath.startsWith(`${candidateDirectory}${path.sep}`)) add(preflight, "unsafe-youtrack-app-path"); else try { const appZip = await readFile(appZipPath); if (appZip.length !== candidate.youtrackApp.bytes || sha256(appZip) !== candidate.youtrackApp.sha256) add(preflight, "youtrack-app-digest-mismatch"); } catch { add(preflight, "missing-youtrack-app"); }
