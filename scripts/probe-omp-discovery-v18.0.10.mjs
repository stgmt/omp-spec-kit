#!/usr/bin/env bun
/**
 * Captures OMP 18.0.10's MCP capability-to-manager handoff without a model turn.
...
const payloadResult = await phase("payload", async () => {
	...
	if (runtimePackage.name !== "@oh-my-pi/pi-coding-agent" || runtimePackage.version !== "18.0.10") {
		throw new Error(`Expected @oh-my-pi/pi-coding-agent@18.0.10, got ${runtimePackage.name}@${runtimePackage.version}`);
	}
...
if (!terminalPhase) {
	const configResult = await phase("capability-config-load", async () => {
		...
		const targetName = loadedNames.find(name => name === packageManifest.name || name.endsWith(`:${packageManifest.name}`));
		if (!targetName || !(targetName in configLoad.sources)) {
			throw new Error(`Converted config/source missing ${packageManifest.name}; loaded: ${loadedNames.join(", ") || "<none>"}`);
		}
...
if (!terminalPhase) {
	const managedQueryResult = await phase("managed-query", async () => {
		const tool = manager.getTools().find(candidate =>
			candidate.mcpServerName === configInspection.targetName && candidate.mcpToolName === "spec_inventory",
		);
...
const receiptResult = await phase("receipt", async () => {
	...
	return {
		...
		provenance: payload ? {
			harness: { path: "scripts/probe-omp-discovery-v18.0.10.mjs", sha256: sha256(harnessText) },
